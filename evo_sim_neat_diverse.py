...
# 保存/復元
LAST10_PATH     = "last10_genomes.json"
SAVE_TOP_K      = 10
...
class Viewer:
    def __init__(self, world: World, win_w=1280, win_h=800):
        pygame.init()
        pygame.display.set_caption("Artificial Life Simulator - NEAT + Fission + Diverse Reseed")
        self.world = world
    ...
    def draw(self):
        ...
        for a in self.world.agents:
            ...
            lx = int(sx + math.cos(ang) * 6); ly = int(sy + math.sin(ang) * 6)
        ...
        txt = (f"t={self.world.t} n={n} meanE={meanE:5.1f} births={self.world.births} "
               f"deaths={self.world.deaths} mode={'FAST' if self.fast else 'NORMAL'}")
        fps = f"FPS={self.clock.get_fps():.1f}"
        info = "[SPACE]Pause [F]Fast [S]Shot [R]Reset [L]Load last10(diverse) [Q]Quit"
        self.screen.blit(self.font.render(txt,  True, (220,220,220)), (8,8))
        self.screen.blit(self.font.render(fps,  True, (180,180,180)), (8,28))
        self.screen.blit(self.font.render(info, True, (160,160,160)), (8,48))
        pygame.display.flip()
            ...
            for ev in pygame.event.get():
                if ev.type == pygame.QUIT:
                    ...
                    if ev.key in (pygame.K_ESCAPE, pygame.K_q):
                        ...
                        self.world = World(from_last10=False)
                    elif ev.key == pygame.K_l:
                        self.world = World(from_last10=True)

            sub = 0 if self.paused else (self.substeps_fast if self.fast else self.substeps_normal)
            if sub>0:
                self.world.tick(substeps=sub)
                # 10秒ごとに last10 を保存
                now = time.time()
                if now - last_save > 10.0:
                    self.world.save_last10()
                    last_save = now
...
# ===================== エントリ =====================
def main():
    world = World(from_last10=os.path.exists(LAST10_PATH))
