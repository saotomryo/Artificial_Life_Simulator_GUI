...
- **行動記録と再生**: GUI から行動ログを録画し、再生や JSON 書き出しが可能。シミュレーションを停止せずに任意のタイミングで記録開始／終了、再生、エクスポートができます。
...
- `記録開始` / `再生` / `記録を保存` で行動記録の開始・停止・再生・エクスポートができます（後述）。
...
### 行動記録と再生
1. `記録開始` を押すと現在のシミュレーションフレーム（統計 + ワールド情報）を順次キャプチャします。録画中は再生・エクスポートボタンが無効化されます。
2. `記録停止`（同じボタン）で録画を終了すると、保存済みフレーム数とともにログ／ステータスバーへメッセージが表示され、再生／エクスポートが有効化されます。
3. `再生` を押すと、シミュレーションを自動で停止したうえで録画済みフレームを GUI に順次描画します。再生停止後は通常操作へ戻ります。
4. `記録を保存` を押すと、録画内容を JSON (`simulation_recording.json` など) に書き出せます。フォーマットは後述。
...
### 3. 行動記録 (`simulation_recording.json` など)
`記録を保存` で出力するファイルは次のような構造です。
...
```json
{
  ...
  "frames": [
    {
      "stats": {"tick": 42, "population": 48, "mean_energy": 180.5, ...},
      "frame": {"width": 2000, "height": 2000, "agents": [...], "foods": [...], "bodies": [...]}
    },
    ...
  ]
}
...
`stats` はコントローラから通知される統計値一式、`frame` はワールドビュー描画に使用している座標／エネルギー情報です。この JSON を別ツールで解析すれば、GUI なしでも行動の推移を後から分析できます。
...
| `simulation_recording*.json` | 行動記録のエクスポート結果（任意）。 |
...
- **記録データのエクスポート**: GUI で録画した行動ログを JSON として保存でき、外部解析や再生用に利用可能です。
